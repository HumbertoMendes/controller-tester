<template>
  <div>
    ...
    <div
      class="game-controller__circle-pad__container"
    >
      <game-controller-circle-pad
        :x-axis="axes[0]"
        :y-axis="axes[1]"
      />
      <game-controller-circle-pad
...
import GameControllerCirclePad from './GameControllerCirclePad.vue';

export default {
  components: { GameControllerCirclePad },
  ...
  methods: {
    getFill(index) {
      console.log(index);
      return this.activeButtons.includes(index) ? 'red' : 'none';
    },
  },
};
</script>
...
.game-controller__circle-pad__container {
  display: flex;
